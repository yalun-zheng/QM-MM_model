...
f = 1
LJParas = {"C":(2.01,0.056*f),"H":(1.34,0.024*f),"O":(1.765,0.1921*f),"N":(1.980,0.070*f),
           "S":(2.050,0.565*f),"P":(2.150,0.585*f),"c":(2.01,0.056*f),"h":(1.34,0.024*f),
           "o":(1.765,0.1921*f),"n":(1.980,0.070*f),"s":(2.050,0.565*f),"p":(2.150,0.585*f)}

def LJPotential(r0,epsilon,r):
    return epsilon*((r0/r)**12-2*(r0/r)**6)
def LJGradient(r0,epsilon,r):
    return 12*epsilon*(r0**6/r**7-r0**12/r**13)
...
#            g += np.linalg.inv(cluster[0].getRotationMatrix(cluster[j+1])).dot(
